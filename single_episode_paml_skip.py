...
if __name__ == "__main__":
	...
	validation_dataset = ReplayMemory(500000)
	...
	if True:
		...
		for i in range(val_num_starting_states):
			val_data = validation_dataset.sample(batch_size, structured=True, max_actions=max_actions, num_episodes=num_episodes, num_starting_states=val_num_starting_states, start_at=i)

			val_states_prev = torch.zeros((batch_size, max_actions, states_dim)).double()
			val_states_next = torch.zeros((batch_size, max_actions, states_dim)).double()
			val_rewards = torch.zeros((batch_size, max_actions)).double()
			val_actions_tensor = torch.zeros((batch_size, max_actions, actions_dim)).double()
			...
			val_loss += P_hat.mle_validation_loss(val_states_next, val_state_actions, pe, 10)
