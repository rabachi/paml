...
class CollectSample(object):
	def __init__(self, env, policy, visualize_rollouts, dt_steps,follow_trajectories):
		self.env = env
		self.policy = policy
	...
	def collect_samples(self, num_rollouts, steps_per_rollout):
		...
		for rollout_number in range(num_rollouts):
			...
			observations, actions, reward_for_rollout = self.perform_rollout(observation, steps_per_rollout, rollout_number, visualization_frequency)
