...
class MBRLNoisySwimmerEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
        dir_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        mujoco_env.MujocoEnv.__init__(self, '%s/mujoco/fixed_swimmer.xml' % dir_path, 4)
        utils.EzPickle.__init__(self)

    def step(self, a):
        ctrl_cost_coeff = 0.0001
        a  = a + (np.linalg.norm(a) * np.random.randn(*a.shape))
