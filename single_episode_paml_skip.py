...
if __name__ == "__main__":
	...
	pe = Policy(n_states, n_actions, continuous=continuous_actionspace, std=0.01)
	...
	lr_schedule = torch.optim.lr_scheduler.MultiStepLR(opt, milestones=[1000,1350,1400], gamma=0.1)
	...
	for ep in range(num_episodes):
		...
		true_pe_grads_attached = grad(true_term, pe.parameters(), create_graph=True, only_inputs=True, allow_unused=True)
		true_pe_grads = [true_pe_grads_attached[i].detach() for i in range(1,len(true_pe_grads_attached))]
		#print(true_pe_grads)
		...
		for i in range(num_iters):
			...
			lr_schedule.step()
			print("ep: {}, paml_loss  = {:.7f}".format(ep, loss.data.cpu()))
